...
func NewAllow(opts *Options) *Allow {
...
func (allow *Allow) ServeHTTP(rw http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
	var (
	...
	if r.Method == "OPTIONS" &&
		...
		headers = allow.opts.PreflightHeader(origin, requestedMethod, requestedHeaders)
	...
	headers = allow.opts.Header(origin)
